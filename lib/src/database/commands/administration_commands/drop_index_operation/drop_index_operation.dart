...
import 'drop_index_options.dart';

const Set keysToOmit = <String>{'name', 'key', 'writeConcern', 'w', 'wtimeout', 'j', 'fsync', 'readPreference', 'session'};

class DropIndexOperation extends CommandOperation {
  Object fieldOrSpec;
  late Map<String, Object> indexes;

  DropIndexOperation(Db db, DbCollection collection, this.fieldOrSpec, DropIndexOptions? indexOptions, {Connection? connection, Map<String, Object>? rawOptions})
      : super(db, <String, Object>{...?indexOptions?.options, ...?rawOptions}, collection: collection, aspect: Aspect.writeOperation, connection: connection) {
    // parseIndexOptions alway returns a filled "keyName" and a "keyFieldHash"
    // elements
    var indexParameters = parseIndexOptions(fieldOrSpec);
    final indexName = options[keyName] != null && options[keyName] is String ? options[keyName] as String : indexParameters[keyName] as String;
  ...
  Map<String, Object> $buildCommand() {
    ...
    return <String, Object>{
      ...
      keyDropIndexesArgument: [indexes[indexes.keys.first]]
    };
  }
}
...
// [['location', '2d'],['type', 1]]
        indexes.add(_fieldIndexName(object[0] as String, (object[1] ?? '1') as String));
